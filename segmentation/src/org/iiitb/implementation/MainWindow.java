...
public class MainWindow extends JFrame implements ActionListener,
	...
	public MainWindow(long size) {
		...
		pane2.setBounds(430, 351, 115, 35);				//change
		pane2.setOpaque(false);

		
		ta = new JTextArea();
		ta.setBorder(BorderFactory.createEtchedBorder());
		ta.setBounds(5,250,175,400);
		ta.setLineWrap(true);
	    ta.setWrapStyleWord(true);
		JScrollPane scrolltxt = new JScrollPane(ta);
		scrolltxt.getPreferredSize();
		add(scrolltxt);		
		...
		status = new JLabel ("This status bar will guide you throughout the simulation");
		...
		CreateProcess = new JButton(
				"<html><font color = blue><body style='width: 10% px'> Create a Process </font> </html>");
		CreateProcess.setBounds(5, 30, 175, 25);

		AddressForProcess = new JButton(
				"<html><font color = blue><body style='width: 10% px'> Generate Address For a Process</font> </html>");
		AddressForProcess.setBounds(5, 145, 175, 50);

		SegmentTable = new JButton(
				"<html><font color = blue><body style='width: 10% px'> Segment Table for a Process</font> </html>");
		SegmentTable.setBounds(5, 90, 175, 50);

		DeleteProcess = new JButton(
				"<html><font color = blue><body style='width: 10% px'> Delete a Process</font> </html>");
		...
		log = new JLabel("Log");		
		log.setBounds(65,220,35,25);
		
		seemore = new JButton("<html> <font color = blue><body style='width: 10% px'>FAQ</font></html>");		//change
		seemore.setBounds(1172, 661,128, 28);
		...
		try {
			ConstantsManual.writer = new PrintWriter("logs_Memory Segmentation.txt", "UTF-8");
			ConstantsManual.writer.println("["+new Timestamp(date.getTime())+"] User selected No. of bits as: "+size+" and therefore a physical memory view of size: "+memorySize+ " is created in this view.");
			logString =  "\n►  User selected No. of bits as: "+size+" and therefore a physical memory view of size: "+memorySize+ " is created in this view.";
			ta.setText(logString);
	...
	public void actionPerformed(ActionEvent e) {
		...
		if (o == CreateProcess) {
			ConstantsManual.writer.println("["+new Timestamp(date.getTime())+"] 'Create a Process' was clicked.");
			logString+= "\n► 'Create a Process' was clicked.";
			ta.setText(logString);
			CreateAProcess cp = new CreateAProcess();
			long[] segmentArray = cp.ProcessInputCode();
			
				
			if (segmentArray[0] == 1) { // User did input something
				...
				if (allocationAddress[1] == -2) { // As all are set to -2

					JOptionPane.showMessageDialog(null, "<html> <font color=red> NOT ENOUGH MEMORY!! </font></html>");
					flag = 0;
					ConstantsManual.writer.println("["+new Timestamp(date.getTime())+"] User was unable to create a process as it exceeded Memory Size i.e. Process Size: " + ConstantsManual.ProcessSizeConstant);
					logString += "\n►  User was unable to create a process as it exceeded Memory Size i.e. Process Size: " + ConstantsManual.ProcessSizeConstant;
					ta.setText(logString);
				}
				...
				if (flag == 1) {
					...
					long processSize=0;
					for (int i = 0; i < 5; i++) {
						MemorySegment memorySegment = new MemorySegment(2,"Segment", true, 10, ResourceType.MEMORY,
					...
					status.setText("Size of the Process " + pid + " is : "+ processSize + ". Size of the Segments are: "+ segmentArray[1]+ " Bytes CONSTANTS, " + segmentArray[2]+ "B GLOBALS, " + segmentArray[3]+  "B CODE, " +segmentArray[4] + "B HEAP, " + segmentArray[5]+ "B STACK. See, Segment Table.");
					ConstantsManual.writer.println("["+new Timestamp(date.getTime())+"] User created a process.");
					logString += "\n►  User created a process";
					ta.setText(logString);
			
					ConstantsManual.writer.println("["+new Timestamp(date.getTime())+"] Size of the Process " + pid + " is : "+ processSize + ". Size of the Segments are: "+ segmentArray[1]+ " Bytes CONSTANTS, " + segmentArray[2]+ "B GLOBALS, " + segmentArray[3]+  "B CODE, " +segmentArray[4] + "B HEAP, " + segmentArray[5]+ "B STACK. ");
					logString += "\n►  Size of the Process " + pid + " is : "+ processSize + ". Size of the Segments are: "+ segmentArray[1]+ " Bytes CONSTANTS, " + segmentArray[2]+ "B GLOBALS, " + segmentArray[3]+  "B CODE, " +segmentArray[4] + "B HEAP, " + segmentArray[5]+ "B STACK. ";
					ta.setText(logString);
					
					if (ConstantsManual.defragmentationFlag==1)
					{
						String tempo = status.getText();
						status.setText("<html><font color=red>  Encountered Defragmentation </font> <font color=black> | "+ tempo + "</font></html>" ); 
						ConstantsManual.writer.println ("["+new Timestamp(date.getTime())+"] Memory Defragmention occured in order to heal the issue of External Fragmentation");
						logString += "\n►  Memory Defragmention occured in order to heal the issue of External Fragmentation";
						ta.setText(logString);
					}
					SegmentView segmentView = new SegmentView(memory);
					getContentPane().add(segmentView).setBounds(740, 0, 600,700);
				...
				ConstantsManual.writer.println ("["+new Timestamp(date.getTime())+"] Available Memory: "+avail);
		...
		if (o == SegmentTable) {
			...
			ConstantsManual.writer.println("["+new Timestamp(date.getTime())+"] User clicked on 'Segment Table' option.");
			logString +=  "\n►  User clicked on 'Segment Table' option.";
			ta.setText(logString);
		...
		if (o == DeleteProcess) {
			// Make segments free
			ConstantsManual.writer.println("["+new Timestamp(date.getTime())+"] User clicked on 'Delete Process' option.");
			logString +=  "\n►  User clicked on 'Delete Process' option.";
			ta.setText(logString);
		...
		if (o == AddressForProcess) {
			...
			ConstantsManual.writer.println("["+new Timestamp(date.getTime())+"] User clicked on 'Generate Address' option.");
			logString +=  "\n►  User clicked on 'Generate Address' option.";
			ta.setText(logString);
		...
		if(o == seemore) {
			...
			JTextArea tf = new JTextArea(40,70);
			 tf.setLineWrap(true);
			    tf.setWrapStyleWord(true);
			   tf.setEditable(false);
			   tf.setText(ConstantsManual.faq);
			    JScrollPane scrollPane = new JScrollPane(tf); 
			    
			    JOptionPane.showConfirmDialog(null, scrollPane, "Information", JOptionPane.DEFAULT_OPTION);
	...
	protected long[] firstFitAllocationPolicy(long[] segmentArray) {
		...
		ConstantsManual.writer.println("["+new Timestamp(date.getTime())+"] Running First Fit Allocation Policy");
		logString +=  "\n►  Running First Fit Allocation Policy.";
		ta.setText(logString);
	
		try {
			allocationAddress = fl.returnSegmentAddresses(segmentArray, flList,  memory);
	...
	public void addprocess() {
		...
		try {
			pid++;
			ConstantsManual.writer.println("["+new Timestamp(date.getTime())+"] Process Count increased to: "+processCount);
			logString +=  "\n►  Process Count increased to: "+processCount;
			ta.setText(logString);
			...
			public void actionPerformed(java.awt.event.ActionEvent e) {
				...
				try {
					...
					myQuery = "SELECT * FROM SegmentTable WHERE P_ID =  "
							+ btnValue;
					ConstantsManual.writer.println("["+new Timestamp(date.getTime())+"] User clicked on Process P"+btnValue);
					logString += "\n►  User clicked on Process P"+btnValue;
					ta.setText(logString);
					...
					if (flag==1)
					{
						...
						int k=0;
						processCount--;
						ConstantsManual.writer.println("["+new Timestamp(date.getTime())+"] Process Count decreased to: "+processCount+".");
						logString += "\n►  Process Count decreased to: "+processCount+".";
						ta.setText(logString);
						z.setText(Integer.toString(processCount));
						...
						while(k<5)
						{
							...
							long avail = fl.updateAvailableMemory(flList, Long.valueOf(result.getString(5)), 1);
							ConstantsManual.writer.println("["+new Timestamp(date.getTime())+"] Available Memory after deletion of "+ConstantsManual.SEGMENT_NAME[k-1]+" Segment from Process P"+btnValue+" is "+avail + " Bytes");
							y.setText(String.valueOf(avail));
							x.setText(String.valueOf(memorySize - avail));
							
							//Adding a node in FreeList
							long endAdd = Long.valueOf (result.getString(4)) + Long.valueOf (result.getString(5));
							fl = new FreeList(Long.valueOf(result.getString(4)), endAdd , Long.valueOf(result.getString(5)), avail);
							flList.add(fl);
							
							//Query to delete segment tables
							String mQuery;
							PreparedStatement preparedSt;
							mQuery = "delete from Process where P_ID="+btnValue;
...
//					
						...
						TextEmpty();
						ProcessDisplay(1);
						//This will regenrate all the process buttons, needed to do this as one process is being deleted
					}
					...
					if (flag==2)
					{
						...
						temp = temp+1; //As the random is inclusive of 0 and exclusive of n
						int m=0;
						while (m<temp)
						{
						result.next();
						...
						if (generateProcRandom==0) //TrueAddress
						{
							cumAdd -= 2;
							status.setForeground(Color.BLACK);
							status.setText("The logical address generated was: "+temp+""+cumAdd+" which means segment "+ temp+ " and byte at location: "+cumAdd+" which, from segment table is valid address");
							ConstantsManual.writer.println("["+new Timestamp(date.getTime())+"] Simulation System generated a valid address: "+temp+""+cumAdd);
							generateProcRandom=1;
						}
						else //ErrorneousAddress
						{
							cumAdd +=2;
							status.setForeground(Color.RED);
							status.setText("Logical address generated was: "+temp+""+cumAdd+" which means segment "+ temp+ " and byte at location: "+cumAdd+", which is Invalid. As the address generated is out of Segment, this is SEGMENTATION FAULT");
							ConstantsManual.writer.println("["+new Timestamp(date.getTime())+"] Simulation System generated an invalid address: "+temp+""+cumAdd);
